    ...
    if not Reg:           
        ...
        if interest[1] in list(i.columns):
            ...
            for j in range(7,10,1):
                ...
                names = [ result.loc[c,'feature'] for c,i in temp.iteritems() if ( (i>=q_0) & (i<=q_1) ) ] 
                print(f'Number of values in the {int(j*10 +1)}st quantile is ', len(vals_inter) )
                try: 
                    avg = sum(vals_inter) / ( float(len(vals_inter)) )
                except:
                    avg = 0.0           
                print('Mean value is ', avg)
                print('This quantile has these features: ', names)
        else:
            temp = i[interest[0]] 
            print(f'Starting with {len(temp)} features.' )
            temp = temp[temp > 0.01]
